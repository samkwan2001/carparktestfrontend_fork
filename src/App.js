...
function App() {
  ...
  const API_BASE_URL = 'https://carparktest3backend.onrender.com:7000';  //明確指明localhost     disadvtage: app.js will send to 用戶之後,
      ...
      function eventMT(){
        ...
        eventSource.onmessage = (event) => {
          ...
          if((
            ...
            console.log("reload");
            // console.log("dont_reload"+dont_reload);
            redirectToNextPage()
          }else
            console.log(
              `
    ...
    const fetchData = () => {     //定義 ===> 
      ...
      getMongo_userState(atob(carNum_base64))  //執行getMongo_userState  ===> 判斷user ga state 返回promise 一旦呢個promise完成 就會執行.then入面ga野(即第一個param) ==>第一個param 即係116-152行
        .then(async(params) => {  
          ...
          document.getElementById("_id").innerHTML = cookie.load("_id")||"";
        });
    };
    ...
    <div className="App">
      <header className="App-header">
        ...
        <div-top id="welcome" style={{ display: "none" }}>
          ...
          <div>
            <button onClick={goto("SelectChargingTime")} >繼續</button>
          </div>
        </div-top>
